...
class MOIP_Transparente_Model_Api
{
        ...
        if ($repeat) {
            ...
            foreach ($duplicate_keys as $key => $value) {
                ...
                if (!array_key_exists($seller_id_temp, $temp_comissionado)) {
                    ...
                    $temp_comissionados[$seller_id_temp] = [
                                                                        'moipAccount' => [
                                                                                                'id' => $seller_id_temp
                                                                                                ],
                                                                        'type' => 'SECONDARY',
    ...
    public function getListaComissoesAvancadas($order)
    {
        ...
        if ($split_type != 'fullstoreview') {
            $comissionados[0] = [
                                'moipAccount' => ['id' => Mage::getStoreConfig('moipall/mktplacet_config/mpa')],
                                'type' => 'PRIMARY',
                            ];
        }
        ...
        if ($split_type == 'attributeproduct') {
            ...
            foreach ($items as $key => $item) {
                ...
                if ((string)$mpa_secundary != '') {
                    $comissionados[] = [
                                                'moipAccount' => ['id' => $mpa_secundary],
                                                'type' => 'SECONDARY',
                                                'amount' => ['fixed' => number_format($comissao_toJson, 2, '', '')]
                                            ];
                }
            }
            ...
            $comissionados[] = [
                    'moipAccount' => ['id' => $mpa_secundary],
                    'type' => 'SECONDARY',
                    'amount' => ['fixed' => number_format($order->getGrandTotal(), 2, '', '')],
                    'feePayor' => 'true'
                ];
        } else {
            // Você pode personalizar seu método de split. falso evento eu mudei o que envio por favor válide... @Denis Barboni

            $splitdata = new Varien_Object(['order' => $order, 'comissionados' => $comissionados]);
            Mage::dispatchEvent('moip_insert_custom_split', ['splitdata' => $splitdata]);
    ...
    public function setDataMoip($order)
    {
        ...
        if ($use_split) {
            ...
            $array_receivers = ['receivers' => $normalize];
    ...
    {
        $post_fields = ['amount' => number_format($amount, 2, '', '')];
        $documento = 'Content-Type: application/json; charset=utf-8';
        if (Mage::getSingleton('transparente/standard')->getConfigData('ambiente') == 'teste') {
