...
class Moip_Transparente_Model_Cron
{
    public function setStatusAll()
    {
        ...
        $orders->addFieldToFilter('created_at', ['gteq' => $from_date])
                         ->addFieldToFilter('created_at', ['lteq' => $to_date])
                         ...
                         ->addAttributeToFilter('payment.method', [['eq' => 'moip_cc'], ['eq' => 'moip_boleto'], ['eq' => 'moip_tef']]);
    ...
    public function setPendingNotification()
    {
        ...
        $orders = Mage::getModel('sales/order')->getCollection()->join(['payment' => 'sales/order_payment'], 'main_table.entity_id=payment.parent_id', ['payment_method' => 'payment.method']);
        $orders->addFieldToFilter('created_at', ['gteq' => $from_date])
                     ->addFieldToFilter('created_at', ['lteq' => $to_date])
                     ->addAttributeToFilter('state', ['eq' => [Mage_Sales_Model_Order::STATE_NEW]])
                     ->addAttributeToFilter('payment.method', [['eq' => 'moip_boleto']]);
            ...
            if ($order->getExtOrderId()) {
                ...
                if (!isset($consult['error'])) {
                    if ($consult['status'] == MOIP_Transparente_Model_Api::MOIP_WAITING) {
                        ...
                        if ($expirationDate) {
                            $details = ['expirationDate' => Mage::app()->getLocale()->date($expirationDate, null, null, true)->toString('dd/MM/Y'), 'boletoHref' => $boletoHref, 'lineCode' => $lineCode];
