<?php
class MOIP_Transparente_Model_Method_Fluxx extends Mage_Payment_Model_Method_Abstract
{
    const METHOD_CODE = 'moip_fluxx';
    ...
    protected $_formBlockType = 'transparente/form_fluxx';
    protected $_infoBlockType = 'transparente/info_fluxx';
    ...
    public function paymentCapture($order, $moip_order, $payment, $moip_payment)
    {
        if ($order->getIncrementId()) {
            ...
            $info = $this->getInfoInstance();
            $additionaldata = unserialize($info->getAdditionalData());
            $additional_data = array('ambiente' => $ambiente, 'moip_order_id' => $moip_order_id, 'moip_pay_id' => $moip_pay_id, 'total' => $total, 'print_href' => $moip_boleto_printHref, 'expiration_date' => Mage::app()->getLocale()->date($moip_boleto_expirationDate, null, null, true)->toString('dd/MM/Y') , 'line_code' => $moip_boleto_lineCode, 'moip_fluxx_id' => $additionaldata['moip_fluxx_id']);
    ...
    public function order(Varien_Object $payment, $amount)
    {
        ...
        $additionaldataAfter = array(
            ...
            'moip_fluxx_id' => $additionaldata['moip_fluxx_id'],
        );
    ...
    public function setJurosDisableMoip($quote)
    {
        if (!$quote->isVirtual()) {
        ...
        $address->setGrandTotal($total + 0);
        $address->setBaseGrandTotal($total + 0);
        $address->save();
        return $this;
    }
    ...
    {
        $this->setJurosDisableMoip($this->getQuote());
        ...
        $additionaldata = array(
            'moip_fluxx_id' => $data->getMoipFluxxId(),
    ...
    public function validate()
    {
        ...
        if ($errorMsg === false) {
            if (!in_array($currency_code, $this->_allowCurrencyCode)) {
                 $errorMsg = MMage::throwException(Mage::helper('transparente')->__('O Moip Não pode Transacionar pedidos feitos em  (' . $currency_code . ') verifique as configurações de Moeda do seu magento.'));
            }
        }
        if ($errorMsg === false) {
                if (!$additionaldata['moip_fluxx_id']) {
                    $errorMsg = Mage::helper('transparente')->__('Informe o código de autorização informado no pedido anterior.');
                }
        }
