...
class MOIP_Onestepcheckout_IndexController extends Mage_Checkout_OnepageController
{
    ...
    {
        $errUrl = Mage::getUrl('*/*/', ['_secure' => true]);
    ...
    public function IdentifyUserAction()
    {
        ...
        if ($customer->getId()) {
            ...
            $result = $result = ['success' => true, 'is_user' => true, 'html' => $output];
        } else {
            $session = $this->getSession();
            $_session = $this->_getSession();
            $_session->setUsername($email);
            ...
            $result = ['success' => true, 'is_user' => false, 'html' => $output->toHtml()];
        }
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
                    ...
                    $result = ['success' => true, 'redirect' => Mage::getUrl('checkout/onepage/', ['_secure' => true])];
                } catch (Mage_Core_Exception $e) {
                    switch ($e->getCode()) {
    ...
    public function NewAddressAction()
    {
        ...
        $result = ['success' => true, 'html' => $output->toHtml()];
    ...
    public function savePaymentMethodAction()
    {
        ...
        try {
            $result = ['success' => true,  'html' => $this->renderTotals()];
        } catch (Exception $e) {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                if (count($errors) === 0) {
                    ...
                    try {
                        if ($update == 'billing') {
                            ...
                            $output = $this->getLayout()
                                ...
                                ->toHtml();
                            $result = ['success' => true, 'update' => 'shipping', 'html' => $output];
                            ...
                            $result = ['success' => true, 'update' => false, 'redirect' => Mage::getUrl('checkout/onepage/')];
                        } else {
                            $result = ['success' => true];
                        }
                        $this->_prepareDataJSON($result);
                        ...
                        $result = ['success' => false, 'update' => $update, 'html' => $e];
                    }
                    ...
                    $result = ['success' => false, 'update' => 'billing', 'error' => $error];
                    $this->_prepareDataJSON($result);
                ...
                $result = ['success' => false, 'update' => 'billing', 'error' => $e->getMessage()];
                $this->_prepareDataJSON($result);
                ...
                $result = ['success' => false, 'update' => 'billing', 'error' => $this->__('Cannot save address.')];
                $this->_prepareDataJSON($result);
    ...
    public function getValidateAddress()
    {
        ...
        if (!$validade) {
            //'id' => 1,
            $url_edit_address = Mage::getUrl('checkout/onepage/EditAddress', ['_secure' => true, 'id' => $billing_id]);
    ...
    {
        $arrayname = ['address_id', 'firstname', 'lastname', 'company', 'email', 'city', 'region_id', 'region', 'postcode', 'country_id', 'telephone', 'fax', 'tipopessoa', 'nomefantasia', 'razaosocial', 'cnpj', 'insestadual', 'save_in_address_book'];
        $filterdata = [];
        if ($filter == 'true') {
        ...
        foreach ($data as $item => $value) {
            if (!is_array($value)) {
                ...
                if (isset($street[0])) {
                    ...
                    $filterdata[$item] = [$street[0], $street_1, $street_2, $street_3];
                }
            }
        }
    ...
    {
        $data_save_billing = $this->getRequest()->getPost('billing', []);
        ...
        try {
            ...
            try {
                ...
                if (count($errors) === 0) {
                    ...
                    $this->_redirectSuccess(Mage::getUrl('checkout/onepage', ['_secure' => true]));
            ...
            $message = $e->getMessage();
            $json_response = ['erros' => 1, 'msg_error' => $message];
